...
calculateJacobi (struct calculation_arguments const* arguments, struct calculation_results* results, struct options const* options)
{
  ...
  uint64_t firstRealRowIndex, lastRealRowIndex;
	...
	double residuum;    /* residuum of current iteration */
	double maxResiduum; /* maximum residuum value of a slave in iteration */
	int const rowToSend = arguments->numberOfRows + 1;
	int const rowToRecieve = 0;
  ...
  {
    firstRealRowIndex = haloindextop + 1;
  }

  if (rank == nprocs - 1) 
  {
    lastRealRowIndex = arguments->numberOfRows;
  }
  else
  {
    lastRealRowIndex = arguments->numberOfRows - 1 ;
  }
  ...
  MPI_Status status;
    ...
    int sendtag = (term_iteration * nprocs) + rankToSendTo;
    int rankToReciveFrom = rank + 1;
    int recvtag = (term_iteration * nprocs) + rankToReciveFrom;

    if (rank == 0)
    { // only receive bottom 
      MPI_Recv(Matrix_In[haloindexbottom], elements, MPI_DOUBLE, rankToReciveFrom, recvtag, MPI_COMM_WORLD, &status);
    }
    else if (rank == nprocs - 1) 
    { // only send top real row
       MPI_Send(Matrix_In[haloindextop], elements, MPI_DOUBLE, rankToSendTo, sendtag, MPI_COMM_WORLD);
    }
    else
    {
      // send top halo receive bottom halo
      MPI_Sendrecv(Matrix_In[haloindextop], elements,
                 MPI_DOUBLE,
                 rankToSendTo,sendtag,
                 Matrix_In[haloindextop], elements, MPI_DOUBLE,
                 rankToReciveFrom,recvtag, MPI_COMM_WORLD, &status);
    }

    rankToSendTo = rank + 1;
    sendtag = (term_iteration * nprocs) + rankToSendTo;
    rankToReciveFrom = rank - 1;
    recvtag = (term_iteration * nprocs) + rankToReciveFrom;
