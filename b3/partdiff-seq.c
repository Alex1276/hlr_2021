	...
	if (options->inf_func == FUNC_F0)
	{
		...
		if(options->termination == TERM_PREC)
		{
			while (options->term_iteration > 0)
			{
				...
				double** matrix1 = Matrix[m1];
				double** matrix2 = Matrix[m2];

				/* over all columns */
				for (i = 1; i < N; i++)
				{
					double* matrix2row_iMinus1 = matrix2[i-1];
					double* matrix2row_i = matrix2[i];
					double* matrix2row_iPlus1 = matrix2[i+1];
					double* matrix1row_i = matrix1[i];
					/* over all rows */
					for (j = 1; j < N; j++)
					{
						star = -matrix2row_i[j-1] - matrix2row_iPlus1[j] - matrix2row_iMinus1[j] + 4.0 * matrix2row_i[j]  - matrix2row_i[j+1];

						residuum = (-star) / 4.0;
						matrix1row_i[j] = matrix2row_i[j] + residuum;
